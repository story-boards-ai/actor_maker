...
export function TrainingImageModal({ image, open, onOpenChange, actorId, actorName, baseImagePath, onImageGenerated }: TrainingImageModalProps) {
  ...
  async function generateTrainingImages() {
    ...
    try {
      ...
      setGenerationProgress({ current: selectedPrompts.length, total: selectedPrompts.length });
      setGenerationMessage(`✅ Generated ${successCount} images${failCount > 0 ? ` (${failCount} failed)` : ''}`);
    ...
    return (
      <Dialog.Root open={open} onOpenChange={onOpenChange}>
        <Dialog.Portal>
          ...
          <Dialog.Content style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', background: 'white', borderRadius: '12px', maxWidth: '900px', width: '90vw', maxHeight: '90vh', overflow: 'auto', zIndex: 1001 }}>
            <div style={{ padding: '32px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                <Dialog.Title style={{ margin: 0, fontSize: '24px', color: '#1e293b' }}>Generate Training Images</Dialog.Title>
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button
                    onClick={selectAll}
                    disabled={generating}
                    style={{ padding: '6px 12px', background: '#f1f5f9', color: '#475569', border: 'none', borderRadius: '6px', fontSize: '12px', fontWeight: 600, cursor: generating ? 'not-allowed' : 'pointer' }}
                  >
                    Select All
                  </button>
                  <button
              ...
              <div style={{ marginBottom: '24px' }}>
                <div style={{ fontSize: '14px', color: '#64748b', marginBottom: '12px' }}>
                  {selectedPromptIds.size} selected • Click tiles to select/deselect
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '12px', maxHeight: '400px', overflowY: 'auto', padding: '4px' }}>
                  {prompts.map((prompt) => {
                    ...
                    const colors = categoryColors[prompt.category as keyof typeof categoryColors] || categoryColors.photorealistic;
                    ...
                    return (
                      ...
                      >
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '8px' }}>
                          <div style={{ fontSize: '13px', fontWeight: 600, color: isSelected ? colors.text : '#1e293b', lineHeight: '1.3' }}>
                            {prompt.label}
                          </div>
                          {isSelected && (
                            <div style={{ fontSize: '16px', color: colors.border }}>✓</div>
                          )}
                        </div>
              ...
              {generationMessage && (
                <div style={{ marginBottom: '16px', padding: '12px', background: generationMessage.startsWith('✅') ? '#f0fdf4' : '#fef2f2', borderRadius: '8px', border: `1px solid ${generationMessage.startsWith('✅') ? '#bbf7d0' : '#fecaca'}`, fontSize: '14px', color: generationMessage.startsWith('✅') ? '#166534' : '#991b1b' }}>
                  {generationMessage}
                  {generating && generationProgress.total > 0 && (
                    <div style={{ marginTop: '8px', background: '#e2e8f0', borderRadius: '4px', height: '6px', overflow: 'hidden' }}>
                      <div style={{ background: '#667eea', height: '100%', width: `${(generationProgress.current / generationProgress.total) * 100}%`, transition: 'width 0.3s ease' }}></div>
                    </div>
                  )}
                </div>
              )}
              ...
              <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
                ...
                <button
                  ...
                  disabled={generating || selectedPromptIds.size === 0 || !baseImagePath}
                  style={{ padding: '12px 24px', background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', color: 'white', border: 'none', borderRadius: '8px', fontWeight: 600, cursor: (generating || selectedPromptIds.size === 0 || !baseImagePath) ? 'not-allowed' : 'pointer', opacity: (generating || selectedPromptIds.size === 0 || !baseImagePath) ? 0.5 : 1, display: 'flex', alignItems: 'center', gap: '8px' }}
                >
                  ...
                  {generating ? `Generating ${generationProgress.current}/${generationProgress.total}` : `✨ Generate ${selectedPromptIds.size} Image${selectedPromptIds.size !== 1 ? 's' : ''}`}
                </button>
              </div>
            </div>
