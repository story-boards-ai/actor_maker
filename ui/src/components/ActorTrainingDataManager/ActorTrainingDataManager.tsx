...
export function ActorTrainingDataManager({
  ...
  const [selectedTrainingImage, setSelectedTrainingImage] =
    useState<TrainingImage | null>(null);
  ...
  const [recreatingImage, setRecreatingImage] = useState<number | null>(null);
  ...
  async function recreateTrainingImage(image: TrainingImage) {
    try {
      setRecreatingImage(image.index);
      setSyncStatus({
        syncing: true,
        progress: { current: 0, total: 1 },
        message: `Recreating ${image.filename}...`
      });

      // Get the prompt from prompt_metadata
      const metadataResponse = await fetch(`/api/actors/${actor.id}/training-data/prompts`);
      if (!metadataResponse.ok) {
        throw new Error('Failed to load prompt metadata');
      }
      
      const metadataData = await metadataResponse.json();
      const imageMetadata = metadataData.prompts.find((p: any) => p.filename === image.filename);
      
      if (!imageMetadata || !imageMetadata.prompt) {
        throw new Error('No prompt found for this image');
      }

      // Extract aspect ratio from filename or metadata (default to 1:1)
      const aspectRatio = imageMetadata.aspect_ratio || '1:1';

      // Call the recreate API endpoint
      const response = await fetch(`/api/actors/${actor.id}/training-data/recreate`, {
        method: 'POST',
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || 'Recreate failed');
      }

      setSyncStatus({
        syncing: false,
        progress: { current: 1, total: 1 },
        message: `✅ Recreated ${image.filename}`
      });

      // Invalidate cache for this image
      invalidateImage(image.s3_url);
      ...
      setSyncStatus({
        syncing: false,
        progress: { current: 0, total: 0 },
        message: `❌ Recreate failed: ${err instanceof Error ? err.message : 'Unknown error'}`
      });
    } finally {
      setRecreatingImage(null);
    }
  }
  ...
  async function deleteAllTrainingData() {
    try {
      ...
      const response = await fetch(
        `/api/actors/${actor.id}/training-data/delete-all`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  async function generateAllPromptImages() {
    try {
      ...
      const response = await fetch(
        `/api/actors/${actor.id}/training-data/generate-all-prompts`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      setSyncStatus({
        ...
        message: `✅ Generated ${result.successful || 0} images (${
          result.failed || 0
        } failed)`,
      });
  ...
  async function deleteTrainingImage(image: TrainingImage) {
    try {
      const response = await fetch(
        `/api/actors/${actor.id}/training-data/delete`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
    ...
    >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
          ...
          <div>
            ...
            <p style={{ margin: "8px 0 0 0", opacity: 0.9, fontSize: "14px" }}>
              ...
              {s3Count} images in S3 • {actor.age}y {actor.sex}{" "}
              {actor.ethnicity}
            </p>
          </div>
        </div>
            ...
            <>
              ...
              <button
                ...
                style={{
                  ...
                  fontWeight: 500,
                  cursor:
                    trainingImages.length === 0 ? "not-allowed" : "pointer",
              ...
              <button
                ...
                style={{
                  padding: "10px 20px",
                  background: trainingDataGood
                    ? "rgba(16, 185, 129, 0.3)"
                    : "rgba(255,255,255,0.2)",
                  color: "white",
                  border: trainingDataGood
                    ? "2px solid #10b981"
      ...
      {syncStatus.syncing && syncStatus.progress.total > 0 && (
        ...
        >
          ...
          >
            {syncStatus.message}: {syncStatus.progress.current} /{" "}
              ...
              >
                ...
                >
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    >
                      This actor doesn't have a base image yet. A base image is
                      required to generate training data for LoRA training.
                      <br />
                      <strong>Actor:</strong> {actor.age}y {actor.sex}{" "}
                      {actor.ethnicity}
                      {actor.description && (
                        <>
                          <br />
            ...
            {trainingImages.length === 0 ? (
              ...
              >
                ...
                <p style={{ margin: 0, color: "#64748b" }}>
                  {baseImage
                    ? "Generate training images from the base image to get started"
                    : "Create a base image first, then generate training images"}
                </p>
              </div>
            ) : (
              <div
                  ...
                  {trainingImages.map((img) => {
                    ...
                    return (
                      <div
                        ...
                        onMouseEnter={() => setHoveredImageIndex(img.index)}
                          ...
                          {hoveredImageIndex === img.index && (
                            <div style={{
                              ...
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  recreateTrainingImage(img);
                                }}
                                disabled={recreatingImage === img.index}
                                style={{
                                  width: "32px",
                                  height: "32px",
                                  padding: 0,
                                  border: "none",
                                  borderRadius: "6px",
                                  background: recreatingImage === img.index ? "#94a3b8" : "#667eea",
                                  color: "white",
                                  fontSize: "16px",
                                  cursor: recreatingImage === img.index ? "not-allowed" : "pointer",
                                  display: "flex",
                                  alignItems: "center",
                                  justifyContent: "center",
                                ...
                                title="Recreate image with same prompt"
                              >
                                {recreatingImage === img.index ? "⏳" : "🔄"}
                              </button>
        ...
        >
          ...
          >
            ...
            >
              ...
              <strong>{trainingImages.length} training images</strong> from both
              local storage and S3. This action cannot be undone.
            </p>
            <div
